...
看历史、稍后再看列表、关注 feed 和空间动态 feed 都是批量输入。推荐输入可使用
`recommendations`、
...
通用 `access_key` 和 TV `tv_access_key` 会作为 OAuth access token 检查。JSON 输出会按凭据
报告 `missing`、`valid`、`rejected` 或 `request_failed` 状态，便于嵌入调用方和自动化使用。
