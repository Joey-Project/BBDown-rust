...
- `RestrictedAreaProxy` supports BBDown/BiliPlus-style playurl proxy endpoints and proxies that
  mirror `api.bilibili.com` path layout.
...
- PGC stream planning tries the official PGC web playurl endpoint first. If it fails and proxy
  candidates are configured, the client tries the ordered proxy chain until one returns a usable DASH
  or FLV stream shape.
- Proxy requests include the generic `Credentials::access_key` when present. The TV-specific access
  key is not reused for PGC proxy fallback.
...
- Resolver diagnostic endpoint fields strip query strings and URL userinfo, so `access_key` values
  and proxy basic-auth credentials are not printed.
- CLI flags:
...
- Mock e2e coverage verifies official PGC failure followed by local proxy success and confirms JSON
  output does not leak the configured access key.
...
- Local gate: `just ci` with formatter check, clippy, 80 library tests, 8 CLI unit tests, 8 mock
