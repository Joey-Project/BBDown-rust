...
- Resolver diagnostic endpoint fields strip query strings and URL userinfo, so `access_key` values
  and proxy basic-auth credentials are not printed. Diagnostic error messages also redact common
  sensitive key-value patterns before JSON output or final access-restricted errors expose them.
- CLI flags:
...
- Local gate: `just ci` with formatter check, clippy, 83 library tests, 8 CLI unit tests, 8 mock
