...
- Updated `Create Release Candidate` so it still runs from the default branch but can release an
  explicit `source_ref`, allowing `v0.2.0-rc.2` to be created from `release/0.2.0` through the
  release GitHub App and tag ruleset path.
...
- Run `Create Release Candidate` from `master` with `version=0.2.0` and
  `source_ref=release/0.2.0`; it should create `v0.2.0-rc.2`.
