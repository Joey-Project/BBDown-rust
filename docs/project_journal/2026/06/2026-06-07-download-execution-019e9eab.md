...
- The executor downloads the first complete DASH video/audio pair for each entry, or FLV `durl`
  segments when DASH media is incomplete and FLV fallback is available.
...
- Safety coverage verifies media downloads do not send cookies, backup URLs are used, matching 416
  ...
  entry directories, and incomplete DASH media uses FLV fallback when available.
- Retry coverage verifies a failed first request can be retried and then written successfully.
- Mux coverage verifies fake ffmpeg success reports, missing or empty mux output failures, stdin
  isolation, and failed ffmpeg status propagation.
...
- Local tests: `cargo test --workspace` with 50 library tests and 5 CLI e2e tests.
