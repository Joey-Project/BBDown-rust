...
switch; it accepts an existing profile or the current default profile as a no-op. Keep using
`CredentialProfileSelection::named(profile)` for a one-request override.
