...
回退路径。分片期间，每个由非基准 CDN 提供的分片都会再从基准 CDN 下载，并在发布临时文件前逐字节
比较，验证请求会与原分片请求并发发出。每个分片 lane 最多同时发出 2 个 Range 请求，因此并行度为 8
时最多会有 16 个并发 Range 请求。这会增加重复传输量和网络负载；内容不一致时会丢弃临时文件并走
常规候选回退。测速和并行传输不保证一定提速；改写 host 也不能证明签名 URL 一定能被另一 CDN 接受。
诊断信息不会记录签名 URL 的 query 字符串。
