...
`avplayer_h264_aac` profile，其中有 `playable`、`preferred`、`score`、codec-family 字段
和机器可读 reason code，方便下游客户端为 AVPlayer/tvOS 优先选择 H.264/AAC 流。cache key
会对 source URL 做 hash，避免暴露明文，同时保留 query-string 的资源身份；更长期的 ABR
grouping 和 cache-retention policy helper 会单独排期。crate 不实现播放任务状态、HLS
