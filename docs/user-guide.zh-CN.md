...
会打印 BiliPlus/BALH-compatible 授权 URL 和 `qr_payload`，然后从 stdin、`--file` 或交互
prompt 读取粘贴的 `balh-login-credentials:` message 或 callback URL/query。它会保存得到
的通用 intl/Bstar access key，且不会打印 token 值。如果输入来自浏览器 `postMessage`，
请使用 `--message-origin`，这样 CLI 会把 sender origin 与本次 login ticket 校验；可信的手
工 callback URL/query 输入不需要该标志。兼容 mock 或部署可以使用 `--auth-base` 和
