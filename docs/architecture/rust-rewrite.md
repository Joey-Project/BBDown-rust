...
`/x/passport-login/oauth2/info` without sending cookies. Probe failures are contained per credential
as `missing`, `valid`, `rejected`, or `request_failed` rather than failing the whole report.
