...
guard 串行化，因此 reclaim 不会删除刚创建的新 writer lock。每次替换 credential file 前会用当前
lock token 做 fencing，释放 lock 时也只有文件内 token 仍属于当前 guard 才会删除。自动 access-key
refresh 在保存前还会在持锁期间验证当前所选 profile 的 access key、access-key provider metadata、
