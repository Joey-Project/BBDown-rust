...
when no decision is provided. `replace` overwrites the planned output root without resume,
`keep-both` writes the next suffixed output root, and `cancel` reports the preflight state without
downloading.
