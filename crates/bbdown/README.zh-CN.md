...
首页推荐、观看历史、关注视频 feed 和空间动态视频 feed。
...
`following` 和空间动态 URL 等批量输入会通过 `ResolvedContent::Collection` 返回；
...
页推荐批次，目前只输出普通视频 `av` 卡片，并在显式 index selection 需要时扩大请求来覆盖
过滤后的条目。观看历史和动态 feed 输入需要在 client credentials 中提供 cookie；观看历史
